...
AlgoSpace is a space shooter game on Algorand with upgradable NFT spaceships and a built-in DSL for you to create for own adventures! Let's first look into the game itself then the DSL.
...
Make sure you have installed AlgoSigner and connected it to the Algorand `Testnet`, then click "Connect your Wallet". AlgoSigner will open to authorize the connection.
![](https://algospace.app/assets/bg-home.png)

The game will then fetch all your spaceship NFTs from the smart contract. If you do not yet have an spaceship NFT, click "Mint New Ship" and AlgoSigner will open to trigger the mint. You will receive a basic ship with entry-level weapons, wings, engine, and cabin. The ship will appear in your list of ships (if not refresh the page). Select that ship to access the game.
...
Algospace is not only a game, it's a metaverse! Players can generate their own adventures by editing the configuration file `dsl.json` which lists the whole configuration of the game and levels. You can create your own galaxy and advantures! Everything you see in the game is configurable.
